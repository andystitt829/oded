<?php
/**
 * Oded functions and definitions.
 *
 ...
 */
	function oded_setup() {
		 ...
		 * If you're building a theme based on Oded, use a find and replace
		 * to change 'oded' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'oded', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'primary' => esc_html__( 'Primary', 'oded' ),
		) );
 ...
 */
function oded_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'oded_content_width', 640 );
}
add_action( 'after_setup_theme', 'oded_content_width', 0 );
...
function oded_scripts() {
	wp_enqueue_style( 'oded-bootstrap', get_template_directory_uri() . '/external-assets/bootstrap.min.css' );

	wp_enqueue_style( 'oded-material', get_template_directory_uri() . '/external-assets/material.min.css' );
...
add_action( 'wp_enqueue_scripts', 'oded_scripts' );
